...
class resHD2D(object):
    ...
    def __init__(self, micro_model, find_obs, filter, interp_method = 'linear'):
        ...
        # Dictionary with stencils and coefficients for finite differencing approx of first order derivatives 
    ...
    def calculate_derivatives_gridpoint(self, nonlocal_var_str, h, i, j, direction, order = 2): 
        """
        ...
        # Recheck this but... should be correct now
        if k in [l for l in range(order-1)]:
            coefficients = self.differencing[order]['fw']['coefficients']
            stencil = self.differencing[order]['fw']['stencil']
        elif k in [N-1-l for l in range(order-1)]:
        ...
        increment = [self.domain_vars['Dt'], self.domain_vars['Dx'], self.domain_vars['Dy']][direction]
        temp = 0 
        for s, sample in enumerate(stencil): 
            idxs = [h,i,j]
            idxs[direction] += sample
            temp += np.multiply( coefficients[s] / increment, self.get_var_gridpoint(nonlocal_var_str, *idxs))
        return temp
...
class resHD_3D(object):
    ...
    def decompose_structures_task(BC, SET , p_filt, h, i, j, k):
        ...
        return [n_t, u_t, eps_t, q_a, s_ab_tracefree, p_t, Pi_res, EOS_res, T_t], [h,i,j,k] #Uncomment if EoS residual is modelled
        # return [n_t, u_t, eps_t, q_a, s_ab_tracefree, p_t, Pi_res, T_t], [h,i,j,k]
